...
answers = final_jah.drop_duplicates(subset=['answer'])['answer'].tolist()
#model = FastText()
...
print(len(final_jah['answer'].tolist()))
print(type(final_jah['answer'].tolist()[0]))
...
error_count = 0
single_word_count = 0
for i, row in final_jah.iterrows():
    answer = row['answer']
    question = row['question']
    context = row['context']
    if len(answer.split(" ")) == 1:
        single_word_answer.append([context,question,answer,row['label']])
        single_word_count += 1
...
df_single_word.to_csv('single_word.csv', index=False)
print(count)
print(error_count)
print(no_count)
